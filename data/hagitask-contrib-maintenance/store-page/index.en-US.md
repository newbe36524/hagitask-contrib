...
`new` and `dev` write only to Contrib. `publish` writes the package source to the local Community checkout without deleting the Contrib source. The installed HagiTask CLI remains protected. Catalog files, detail JSON, ZIP archives, and other generated output are excluded.
